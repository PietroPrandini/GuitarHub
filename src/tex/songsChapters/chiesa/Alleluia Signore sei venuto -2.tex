%	Uncomment the next two lines and the last twice
%	if the song is over 2 pages
%\renewcommand{\lyricfont}{\small}
%\renewcommand{\printchord}[1]{\bf#1\small}

\beginsong{%Title
Alleluia Signore sei venuto}[
%by={} % Authors, composers, and other contributors
,cr={\centering{\href{https://github.com/PietroPrandini/GuitarHub}{https://github.com/PietroPrandini/GuitarHub} - \href{http://creativecommons.org/licenses/by-sa/4.0/}{CC-BY-SA} - \filemodprintdate{"tex/songsBodies/Alleluia Signore sei venuto.tex"}}}, % Copyright information
%,sr={} % Related scripture references
%,index={} % An extra index entry for a line of lyrics
%,ititle={} % An extra index entry for a hidden title
]

%\capo{0}
\transpose{10} % Autoimatic transpositions from +0 to +12 semitones

\input{"tex/songsBodies/Alleluia Signore sei venuto.tex"}

\endsong

%\renewcommand{\lyricfont}{}
%\renewcommand{\printchord}[1]{\bf#1}
